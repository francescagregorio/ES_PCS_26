...
class undirected_graph{
    ...
    public:
        ...
        void add_edge (const T& u, const T& v){
            ...
            if (std::find(neighbours_from.begin()+1, neighbours_from.end(), to ) == neighbours_from.end()){
                //se non c'è, inserisco to e riordino la parte di vettori corrispondente ai vicini di from
                neighbours_from.push_back(to);
                std::sort(neighbours_from.begin() +1, neighbours_from.end());
            }
            //ai vicini di to bisogna aggiungere from, se non c'è già
            auto& neighbours_to = m_adj[idx_to];
            if (std::find(neighbours_to.begin()+1, neighbours_to.end(), from) == neighbours_to.end()){
                neighbours_to.push_back(from);
                std::sort(neighbours_to.begin()+1, neighbours_to.end());
